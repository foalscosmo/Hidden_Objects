...
namespace GardenScripts
{
    public class HintManager : MonoBehaviour
    {
        ...
        private void HandleHintObject()
        {
            ...
            ScaleUpAndDown(touchObject.TargetObjects[hintIndex].transform);
        }
        ...
        private void ScaleUpAndDown(Transform target)
        {
            ...
            target.DOScale(originalScale + new Vector3(0.0001f, 0.0001f, 0.0001f), 0.5f).OnComplete(() =>
            {
                target.DOScale(originalScale, 0.5f).OnComplete(() =>
                {
                    target.DOScale(originalScale + new Vector3(0.0001f, 0.0001f, 0.0001f), 0.5f).OnComplete(() =>
                    {
                        target.DOScale(originalScale, 0.5f);
                    });
                });
            });
        }
